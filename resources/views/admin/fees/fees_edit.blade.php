        ...
        <div class="container mt-4">
            ...
            <form action="{{ route('admin.update_fees',$fees->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="cnic" id="cnic" class="form-control" value="{{ $fees->cnic }}"
                        required>
                    @error('cnic')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="amount" id="amount" class="form-control" value="{{ $fees->amount }}"
                        required>
                    @error('amount')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <select name="status" id="status" class="form-control form-control" required>
                        ...
                        <option value="pending" {{ $fees->status == 'pending' ? 'selected' : '' }}>Pending</option>
                        <option value="inprogress" {{ $fees->status == 'inprogress' ? 'selected' : '' }}>In Progress
                        </option>
                        <option value="complete" {{ $fees->status == 'complete' ? 'selected' : '' }}>Complete</option>
                    </select>
                    @error('status')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
